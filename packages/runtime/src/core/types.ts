...
export interface Instruction {
  ...
  itemFns?: string[] // For用: テキストノード内の各動的分を更新するための関数の文字列表現
...
// Built-in component types for JSX
export type ForComponent<T> = (props: { each: () => T[] }, children: (item: () => T) => VNode
export type ShowComponent = (props: { when: () => boolean }, children: VNode) => VNode
import type z from 'zod'

...
import type { Instruction, Manifest } from '@quix/runtime'
...
export function generateQuixOutput(manifest: Manifest): string {
  ...
  for (const [signalId, instructions] of Object.entries(updateMap)) {
    ...
    instructions.forEach((inst, i) => {
      ...
      targetDeclarations.push(
        `const ${varName} = document.querySelector(".${markerId}");`
      )
...
<body>
  ...
  <script type="module">
    ...
    const state = ${JSON.stringify(
      Object.fromEntries(
        Object.entries(stateBank)
          .filter(([_, e]) => !e.isDerived)
          .map(([k, e]) => [e.id, e.value])
      )
    )};
